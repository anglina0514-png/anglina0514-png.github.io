...
initScrollVideoStage();
initAmbientParticles();
initFixedCardsReveal();
updateScrollState();
...
function initScrollVideoStage() {
  const canvas = document.getElementById("scrollVideoCanvas");
  const video = document.getElementById("scrollVideoFallback");
  ...
  const scrollProgress = () => {
    ...
    return clamp((window.scrollY - window.innerHeight * 0.28) / (max - window.innerHeight * 0.28), 0, 1);
  };
  ...
  const draw = () => {
    ...
    if (metadataReady && video.readyState >= 2) {
      ...
      const target = duration ? progress * Math.max(0.01, duration - 0.08) : 0;

      if (!seeking && Math.abs(video.currentTime - target) > 0.035) {
        seeking = true;
        video.currentTime = target;
      }

      if (Math.abs(video.currentTime - lastDraw) > 0.015 || lastDraw < 0) {
...
function initFixedCardsReveal() {
  const fixedCards = document.getElementById("fixed-cards");
  const grid = fixedCards?.querySelector(".fixed-card-grid");
  const trigger = document.getElementById("news");
  if (!fixedCards || !grid || !trigger) return;

  const tick = () => {
    const rect = trigger.getBoundingClientRect();
    const top = rect.top + window.scrollY;
    const height = rect.height;
    const start = top - window.innerHeight * 0.45;
    const end = top + height - window.innerHeight * 0.3;
    const range = Math.max(1, end - start);
    const progress = clamp((window.scrollY - start) / range, 0, 1);
    const fadeIn = clamp((window.scrollY - (start - window.innerHeight * 0.18)) / (window.innerHeight * 0.2), 0, 1);
    const fadeOut = clamp((end + window.innerHeight * 0.32 - window.scrollY) / (window.innerHeight * 0.32), 0, 1);
    const opacity = Math.min(fadeIn, fadeOut);
    const reveal = progress * 128;

    fixedCards.style.opacity = opacity.toFixed(3);
    fixedCards.style.pointerEvents = opacity > 0.2 ? "auto" : "none";
    const direction = window.innerWidth < 768 ? "to bottom" : "to right";
    const width = window.innerWidth < 768 ? 20 : 15;
    const mask = `linear-gradient(${direction}, black ${reveal}%, transparent ${reveal + width}%)`;
